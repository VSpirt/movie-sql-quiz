...
select t.decade, count(t.title) as cnt, avg(t.budget) as "average budget", avg(t.gross) as "average gross"
...
-- were the lead actors?
   ...
   select top(5) title, yr, gross, a.name
   from movie m
...
select name from actor
where id = 
	(Select top(1) actorid from casting
	where movieid in
